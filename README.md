# 实验室室内质控管理工具

## 项目简介

这是一个基于 Python、Streamlit、SQLite、pandas 和 matplotlib 的本地室内质控工具，当前版本包含以下页面与模块：

- `Main` 欢迎页：用于说明产品定位、方法入口、快速开始和使用教程
- `LJ` 模块：单水平 LJ 曲线质控主流程
- `Z-score` 模块：双水平 / 三水平多水平 IQC 主流程
- `Instant` 页面：即时法工作台，支持单水平累计、SI 判定与转入 LJ 前的试用流程

当前版本适合作为内部试用、演示和小范围部署的基线版本。

## 当前功能概览

### LJ

- 项目与批次管理
- 批次级 CV 要求（%）保存与建靶提醒
- 建靶与正式质控
- Westgard 规则判读
- LJ 图查看
- 最新结果分析与规则汇总
- 检测记录维护
- 当前批次 Excel / CSV 导出
- 当前 LJ 图 PNG 导出
- 月度质控图 PNG 导出

### Z-score

- 项目级双水平 / 三水平配置
- 批次级水平说明配置
- 批次级 CV 要求（%）保存与建靶提醒
- 多水平检测记录级录入
- 建靶期 / 正式质控期管理
- 单水平图 / 合并图
- 建靶期图 / 正式质控图 / 全图切换
- 厂家参考值手工录入位
- 最新结果分析
- 正式期实时统计
- 检测记录维护与整批次重算

### Main

- 正式欢迎界面
- LJ / Z-score / Instant 方法入口说明
- 快速开始路径
- 结构化使用教程
- 测试版更新说明与手工测试步骤
- 当前版本边界说明

### Instant

- 单水平即时法累计
- 3 个有效点后开始 SI 判定
- 20 个有效点后提示可人工确认转入 LJ
- 支持疑似离群提示与手工处理备注

## 当前限制 / 未完成项

- 导入入口当前仅支持标准 CSV 模板，不支持 Excel 导入
- 跨批次批量导入未完成
- COA 解析未完成
- peer-group 数据未完成
- `target freeze / re-establish` 等高级流程未完成

## 更新记录

### 2026-04-05 更新：数据的导入导出

#### 本轮改动

- LJ：当前批次已支持按阶段导出建靶期 / 正式期数据，格式可选 Excel / CSV；已支持当前 LJ 图 PNG 导出，以及仅基于正式数据的月度质控图 PNG 导出
- LJ：已提供建靶期 / 正式期 CSV 模板下载、CSV 审查与确认导入；审查结果会返回总行数、可导入行数、错误行数、警告行数，并展示逐行问题
- Z-score：当前批次已支持按阶段导出建靶期 / 正式期 run 宽表，格式可选 Excel / CSV；已支持当前图 PNG 导出，以及正式期月度图 PNG 导出
- Z-score：已提供建靶期 / 正式期 CSV 模板下载、CSV 审查与确认导入；模板会按当前批次 2 水平 / 3 水平自动生成，审查结果同样返回摘要与逐行问题
- 导入审查：阻断错误会禁止确认导入；非阻断项保留为提醒，覆盖模板不匹配、缺少必填列、模板外列、检测时间重复、备注为空等常见情况

#### 当前限制 / 已知边界

- 导入入口当前仅支持标准 CSV 模板，不支持 Excel 导入，也不支持跨批次批量导入
- LJ 月度质控图与 Z-score 月度图当前都只导正式期数据，日期范围最长 30 天
- Z-score 建靶期导入仅在当前批次未完成建靶时开放；正式期导入需建靶完成后再执行，建靶期文件不能跨阶段直接导入到正式期
- 导入为追加写入当前批次；确认导入后会按现有业务口径刷新统计、判定、图表与最新结果分析

### 2026-04-01

#### 本轮改动

- LJ 与 Z-score 已接入手动备注字段：维护区可查看和编辑，图上对含备注的点增加描边提示
- LJ 新增结果录入支持备注；Z-score 备注入口统一为“最新结果分析”下快捷补备注 + 维护区编辑，录入区不再保留常驻备注输入框
- Z-score 最新结果分析改为按检测序号取最新 run，修复“图上已判异常，但卡片显示无规则触发”的不一致
- Z-score 检测人输入改为与 LJ 一致的最近使用记忆模式，支持下拉选择最近姓名，也支持手动输入新姓名
- 修复旧库升级后 batches 子表仍引用 `batches_legacy` 的问题，`create_zscore_batch(...)` 不再因外键残留报错

#### 手工测试

- LJ：新增一条结果并填写备注，保存后检查维护区显示与图上描边；新增一条不填备注的结果，确认不报错且图上不加描边
- LJ：在维护区修改已有备注，确认刷新后仍存在；若最新结果为警告或失控，从“最新结果分析”下快捷补备注，确认维护区和图上同步
- Z-score：录入区不再显示常驻备注输入框；检测人支持从最近使用列表下拉选择，也支持直接输入新名字，保存后新名字会进入最近使用列表
- Z-score：新增一条 run 并保存，确认不依赖录入区备注也能正常保存；在维护区查看和编辑备注，确认刷新后仍存在，图上描边状态同步
- Z-score：当最新 formal run 为 `warning` 或 `reject` 时，从“最新结果分析”下快捷补备注，确认维护区和图上同步，不再保留第三个分散入口
- Z-score：选取一个最新异常 run，确认图上最后一个 run 的状态样式与“最新结果分析”卡片中的规则判定一致，不再出现“图上像失控，但卡片显示无规则触发”
- 旧库升级：使用旧库副本执行数据库初始化后，新建 Z-score 批次，确认不再出现 `main.batches_legacy` 相关报错
- 回归一遍现有 LJ 主流程与当前实际生效的 Z-score 主流程

#### 当前注意事项

- 本轮未新增图上点选交互，备注全文仍只通过维护入口或异常快捷入口查看与修改
- Z-score 录入区检测人新交互已完成代码接入，但尚未单独做一次完整浏览器手点回归
- `database.py` 前半段旧 Z-score 重复定义仍保留，继续以后半段实际生效定义为准

## 运行方式

### 安装依赖

```powershell
python -m pip install -r requirements.txt
```

### 启动应用

直接运行 Streamlit 入口：

```powershell
python -m streamlit run app.py --server.headless true --server.port 8506
```

也可以使用启动脚本：

- `run_app.py`
- `run_app.bat`

### 入口文件

- 主页面入口：`app.py`
- 打包后启动入口：`run_app.py`

### 数据库初始化

- 应用启动时会自动执行数据库初始化，无需手工建表
- 数据库默认持久化位置：
  - Windows：`%LOCALAPPDATA%\LJQCApp\qc_lj_app.db`
  - 无 `LOCALAPPDATA` 时：`~/.ljqcapp/qc_lj_app.db`
- 如需清空试用数据，可运行：

```powershell
python reset_db.py
```

也可以使用：

- `reset_db.bat`

## 演示数据与月报展示

演示数据仅用于培训、现场展示和功能验收，不用于真实室内质控。所有新演示项目名称均以 `【演示】` 开头，批次信息中包含“仅演示，请勿用于真实质控”的说明。

### full profile

`full` profile 会导入 8 套演示数据：

- `【演示】Instant-01 20点累计-可转入LJ`
- `【演示】Instant-02 20点累计-SI疑似离群`
- `【演示】LJ-MR-01 两个月月报-稳定运行`
- `【演示】LJ-MR-02 两个月月报-含失控与纠正措施`
- `【演示】Z2-MR-01 双水平两个月月报-稳定运行`
- `【演示】Z2-MR-02 双水平两个月月报-含失控与纠正措施`
- `【演示】Z3-MR-01 三水平两个月月报-稳定运行`
- `【演示】Z3-MR-02 三水平两个月月报-含失控与纠正措施`

其中 LJ、Z-score 双水平、Z-score 三水平月报展示批次均包含：

- 2026-03-01 至 2026-03-20：20 个有效建靶点 / 20 次建靶 run
- 2026-04：20 条正式期记录 / 20 次正式期 run
- 2026-05：20 条正式期记录 / 20 次正式期 run

含失控与纠正措施的数据组已预置 `manual_note`，可在月报“异常/失控记录表”和“原因与纠正措施”中展示。

### 快捷命令

导入 full profile 演示数据：

```powershell
py tools\seed_demo_data.py --profile full --replace-demo
```

也可以双击：

- `seed_demo_data.bat`

只删除 `【演示】` 前缀数据，不影响真实项目：

```powershell
py tools\seed_demo_data.py --delete-demo
```

也可以双击：

- `delete_demo_data.bat`

全库重置后导入 full profile：

```powershell
py tools\seed_demo_data.py --profile full --reset-all --yes-reset-all
```

也可以双击：

- `reset_and_seed_demo_data.bat`

注意：全库重置会删除当前配置数据库中的所有数据，bat 脚本会要求输入 `RESET` 确认。

### 月报展示路径

导入演示数据后，进入 LJ 月报或 Z-score 月报区域，选择以下演示批次之一：

- `【演示】LJ-MR-01 两个月月报-稳定运行`
- `【演示】LJ-MR-02 两个月月报-含失控与纠正措施`
- `【演示】Z2-MR-01 双水平两个月月报-稳定运行`
- `【演示】Z2-MR-02 双水平两个月月报-含失控与纠正措施`
- `【演示】Z3-MR-01 三水平两个月月报-稳定运行`
- `【演示】Z3-MR-02 三水平两个月月报-含失控与纠正措施`

月份选择 `2026年04月` 或 `2026年05月`，均可生成月报预览并下载 PDF。导入时也会为这些月报展示批次预置报告历史快照，便于展示报告历史页的追溯与下载能力。

## 使用说明摘要

### LJ

1. 新建项目
2. 新建批次并确认建靶所需次数
3. 录入检测结果
4. 建靶完成后自动进入正式质控
5. 查看图表、规则汇总和最新结果分析
6. 如需修正历史数据，使用记录维护入口

### Z-score

1. 新建项目，并确定双水平或三水平
2. 新建批次并填写各水平说明
3. 录入多水平检测记录
4. 全部水平达到建靶条件后进入正式质控
5. 查看单水平图、合并图和最新结果分析
6. 如需修正正式期记录，使用记录维护入口，系统会自动整批次重算

### 测试版更新：批次级 CV 要求（%）

#### 更新内容

- LJ 与 Z-score 的新建批次均支持可选填写“CV 要求（%）”
- 当前已有的编辑批次入口支持回显和修改“CV 要求（%）”
- 该值保存为批次属性，工作区只读取已保存值做提醒，不提供第二套临时输入入口
- LJ：建靶过程中一旦已有足够数据可计算 SD/CV，就开始显示“当前累计 CV%”与“批次要求”的对照提醒
- Z-score：建靶过程中按各水平分别显示 provisional CV% 与“批次要求”的对照提醒
- 提醒只作提示，不阻断保存，不改变现有判读逻辑与建靶/正式期切换逻辑

#### 使用方法

1. 在“项目与批次管理”中先选择项目，再新建批次
2. 新建批次时可选填写“CV 要求（%）”；留空则表示该批次不启用此提醒
3. 如需调整，可在当前已有的编辑批次入口修改并保存
4. 进入工作区后，系统只读取该批次已保存的 CV 要求并显示提醒，不需要也不能在工作区再次录入
5. 当批次未设置 CV 要求时，系统保持兼容，不报错，也不强制显示空提示

#### 手工测试步骤

1. LJ：新建批次时填写 CV 要求，例如 `5.00`，保存后进入当前批次页
2. LJ：连续录入至少 2 条建靶结果，确认开始显示“当前累计建靶 CV% / 批次要求 / 是否满足要求”
3. LJ：新建批次时不填写 CV 要求，进入工作区确认不报错、无空提示
4. LJ：通过编辑批次修改 CV 要求后再次进入工作区，确认提醒读取的是修改后的已保存值
5. Z-score：新建批次时填写 CV 要求，录入多水平建靶 run，确认各 level 分别显示 provisional CV% 对照提醒
6. Z-score：旧批次或未设置 CV 要求的批次进入工作区，确认不报错
7. 回归一遍现有 LJ 主流程
8. 回归一遍现有实际生效的 Z-score 主流程

## 测试说明

当前项目至少包含以下检查方式：

- 语法检查：

```powershell
python -m py_compile app.py database.py zscore_logic.py zscore_plotting.py tests/zscore_smoke_test.py qc_logic.py plotting.py
```

- Z-score smoke tests：

```powershell
python tests/zscore_smoke_test.py
```

`tests/zscore_smoke_test.py` 当前覆盖的重点包括：

- 双水平 / 三水平规则组合
- 建靶期与正式质控期切换
- 检测记录维护后的整批次重算
- 图表基础输出与图例
- 厂家参考值与正式期实时统计

## 版本状态

- 当前版本已经具备 LJ 与 Z-score 主流程，可用于内部试用、演示和小范围部署
- Z-score 主干流程已经落地，包括多水平录入、建靶、正式质控、图表与维护
- 仍有部分增强功能处于后续计划中，例如批量导入、COA 解析、peer-group 和高级靶值流程

## 相关脚本

- `packaging/build_exe.bat`：打包入口
- `packaging/LJQCApp.spec`：PyInstaller 配置
- `run_app.py` / `run_app.bat`：应用启动脚本
- `reset_db.py` / `reset_db.bat`：数据库重置脚本
