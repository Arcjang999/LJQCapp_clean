...
def render_instant_v12_configuration_selection(manage_tab, projects, project_id, batches, batch_id, issues):
    with manage_tab:
        ...
        if left.button("打开项目/批次管理", key="instant_open_v11_project_management"):
            open_global_page("show_project_management_page")
        if right.button("打开基础资料", key="instant_open_v11_master_data"):
        ...
        if projects.empty:
            st.info("当前没有可用的即时法配置。请先在项目/批次管理中建立并启用项目模板与批号配置。")
            return
        ...
        with project_col:
            ...
            if project_map[label] != project_id:
                ...
                st.session_state["v12_instant_batch_selector"] = "请选择已启用的批号配置"
                st.rerun()
        with batch_col:
            label = st.selectbox("批号配置", list(batch_map), index=_selector_index(batch_map, batch_id), key="v12_instant_batch_selector", disabled=project_id is None)
        ...
        if batch_id is None:
            ...
            st.dataframe(display[["project_name", "v11_config_name", "lot_no", "instrument", "reagent", "qc_material", "unit_symbol", "method_name", "target_n", "transfer_status"]].rename(columns={
                "project_name": "检验项目", "v11_config_name": "配置名称", "lot_no": "质控品批号",
