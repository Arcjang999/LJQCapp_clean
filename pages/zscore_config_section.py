...
def render_zscore_v12_configuration_selection(manage_tab, projects, project_id, batches, batch_id, issues):
    with manage_tab:
        render_section_intro(
            title="Z-score 项目与批号选择",
            caption="请选择已启用的 2 或 3 水平配置。可按本批建靶，或在靶值版本中确认全部水平的控制参数。",
            badges=["多水平", "参数版本"], tone="accent",
        )
        left, right = st.columns(2)
        if left.button("打开项目/批次管理", key="zscore_open_v11_project_management"):
            open_global_page("show_project_management_page")
        if right.button("打开基础资料", key="zscore_open_v11_master_data"):
            open_global_page("show_master_data_page")
        if issues:
            st.warning(f"有 {len(issues)} 个配置尚不能进入工作台。")
            with st.expander("查看暂未接入的配置"):
                st.dataframe(pd.DataFrame(issues).rename(columns={"config_name": "配置名称", "test_item_name": "检验项目", "lot_no": "质控品批号", "issue": "说明"}), hide_index=True)
        if projects.empty:
            st.info("当前没有可用的多水平配置。请先在项目/批次管理中建立并启用项目模板与批号配置。")
            return
        ...
        with project_col:
            ...
            if project_map[label] != project_id:
                ...
                st.session_state["v12_zscore_batch_selector"] = "请选择已启用的批号配置"
                st.rerun()
        with batch_col:
            label = st.selectbox("批号配置", list(batch_map), index=_selector_index(batch_map, batch_id), key="v12_zscore_batch_selector", disabled=project_id is None)
            if batch_map[label] != batch_id:
                st.session_state["zscore_selected_batch_id"] = batch_map[label]
                st.rerun()
        if batch_id is None:
            st.info("请选择批号配置后进入“当前批次”。")
            ...
            st.dataframe(batches[["project_name", "v11_config_name", "lot_no", "level_count", "instrument", "reagent", "qc_material", "unit_symbol", "method_name", "target_n"]].rename(columns={
                "project_name": "检验项目", "v11_config_name": "配置名称", "lot_no": "质控品批号", "level_count": "水平数",
