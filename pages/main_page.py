...
3. 创建批次，核对各检验项目的 CV 要求上限及依据，选择实际质控品批号和全部水平，设置建靶数量或靶均值与 SD，检查后启用。靶值 CV% 自动计算；CV 要求留空表示未设置。
4. 使用人工或厂家靶值时，还需在“批号使用与追溯 → 靶值版本”确认全部水平参数、依据、确认人及生效时间。
5. 在“系统设置”填写实验室和报告信息。完成准备后，进入对应方法工作台选择项目与批次。

每个检验项目使用一种输入值类型：真实检测值、Ct 值或 log 值。录入与导入时按检验项目所选类型填写。
""",
        "单水平 LJ：录入、建靶与日常判读": """
1. 进入“单水平（LJ法）”，选择已启用的项目与批次，再打开“当前批次”。
2. 核对仪器、单位和质控品批号，填写检测时间、检测人、检测值，并选择本次实际试剂批号。
3. 本批次建靶时，查看有效点数、均值、SD、CV 和疑似离群提示。系统不会自动剔除记录，可在维护区保留、禁用或恢复。
4. 建靶达到设定数量后，后续检测按正式参数判读。使用已确认的人工或厂家参数时，按参数生效时间进行判读。
5. 保存后查看 LJ 图和规则提示；月底在月报区选择月份，核对记录和异常处理情况，再生成 PDF。
""",
        "多水平 Z-score：一次录入全部水平": """
1. 选择已启用的 2 或 3 水平项目与批次，核对各水平的名称和实际质控品批号。
2. 一次检测同时填写全部水平结果，选择实际试剂批号后保存。
3. 建靶期分别查看各水平统计；保留、禁用和恢复按整次检测联动处理。
4. 全部水平完成建靶或确认正式参数后，查看整次检测的联合结论。任一水平触发拒绝规则时，整次检测判为失控。
5. 可切换单水平图与合并图。进入正式期后，建靶记录只读；月报可在当前批次的月报区生成。
""",
        "即时法：积累数据并确认转入 LJ": """
1. 选择已启用、采用本批次建靶的单水平即时法项目与批次，逐次录入检测结果和实际试剂批号。
2. 有 3 个有效点后开始格拉布斯检验，并显示 SI 提示。疑似离群点需人工复核，系统不会自动剔除。
3. 累计到 20 个有效建靶点后，核对数据并按提示人工确认转入 LJ；系统不会自动转换。
...
- **质控品换批**：先在基础资料登记新批号及水平，再从原配置创建新批并行配置。新批从空记录开始，旧批保留；参数和各实际批号的验证全部确认后正式启用。
- **只换部分水平**：建立新的完整水平组合，核对每个水平的实际批号。未换水平的参数可作参考，仍需与新水平一起确认。
- **结束旧批**：核对结束时间后将旧批设为“结束使用”。结束后可查询、导出和查看报告，不能再录入或维护。
...
历史批次可在“批号使用与追溯 → 历史记录”查询，包括已停用或结束使用的批次。缺失的历史批号保持“未记录”。
""",
        "月报与报告历史": """
...
def render_main_entry_page() -> None:
    render_section_intro(
        title="日常质控",
        caption="选择质控方法，进入项目与批次，开始录入或查看结果。",
        tone="accent",
    )
    cards = [
        ("单水平", "LJ 法", "适用于单水平质控，查看 LJ 图、Westgard 判读和月度报告。",
         ["支持本批次建靶及已确认的人工、厂家参数。", "记录实际批号，保留参数变更前后的检测历史。"],
        ...
        ("单水平", "即时法", "从少量有效点开始观察，逐步积累建靶数据。",
         ["3 个有效点后开始格拉布斯检验和 SI 提示。", "20 个有效建靶点后可人工确认转入 LJ。"],
