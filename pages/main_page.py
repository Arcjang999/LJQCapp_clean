    ...
    with st.expander("第一次使用：先做什么", expanded=True):
        st.markdown(
            dedent(
                ...
                4. 如果现在数据还少，暂时凑不够 20 个点，选择“即时法”先过渡。
    ...
    with st.expander("多水平（Z-score法）：一步一步怎么做"):
        st.markdown(
            dedent(
                ...
                3. 进入“项目/批次”，建立并启用 Z-score 项目模板，选择 2 或 3 水平。
                4. 建立批号配置，为全部水平选择“本批次建靶”，保存并启用。
                5. 回到“项目与批号”，选择配置并确认单位、方法、各水平名称和建靶数量。
                6. 进入当前批次页面。
    ...
    with st.expander("即时法：一步一步怎么做"):
        st.markdown(
            dedent(
                ...
                1. 在右上角“项目/批次”中，新建或选择项目模板；仪器、试剂和质控品从基础资料选择。
                2. 为检验项目选择“即时法”、输入值类型、单位和检测方法，固定为单水平、20 个有效点。
                3. 建立质控品批号配置，分配一个水平，选择“本批次建靶”并启用；再进入“即时法”选择该项目和批号。
