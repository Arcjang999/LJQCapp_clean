...
2026-09-21 需求合并提示：本文保留 V1.1 实施基线；新要求见 [需求合并稿](requirements_merged_2026_09_21.md) 和 [基础资料增量](project_material_workflow_alignment.md#8-2026-09-21-面聊合并后的基础资料增量)。厂家按三类业务分开维护与选择，不能由下文共用厂家表推导出允许混选；panel 批量配置继续复用；平行换批的 20 点要求在任务 3A 落实，不由下文通用建靶范围覆盖。上述增量尚未实施，不修改本文历史完成记录。

状态：已实现规格 v0.3
V1.2 补充（2026-09-11）：新建模板入口已增加必选的试剂字典字段与模板默认试剂；多项目逐项试剂配置仍保留。增量规则及兼容策略以 [模板试剂补充规格](v1_2_template_reagent_spec.md) 为准。
日期：2026-09-02
代码基线：main 3c737fb
...
V1.2 创建页面同时选择本地仪器、试剂和质控品，新增可空外键 `default_reagent_id` 作为后续项目的预填值；旧模板可补选，不覆盖各项目的试剂或历史批号配置。
