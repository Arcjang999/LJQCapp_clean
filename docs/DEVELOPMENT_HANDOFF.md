...
当前用户仅要求文档合并与评审，不开始开发。本轮保留已有未提交文档，只修改 Markdown；未启动应用、迁移或测试，未操作真实用户数据库。下列现场、哈希及验证信息来自此前交接，本轮未重新运行这些检查，不能当作新增需求已完成的证据。
...
本次停留在需求合并；后续获得继续开发指令时，按 `AGENTS.md` 的顺序读完交接资料，再读 [术语与流程口径](terminology_and_workflow_alignment.md)、[界面用语对照](ui_wording_manual_alignment.md)、[下一阶段任务书](NEXT_SESSION_TASKS.md) 和 [需求合并稿](requirements_merged_2026_09_21.md)。既定开发主线为：
...
新增基础资料关系是任务 2 完整流程的依赖，已拆为任务 2A；独立临时质控为 2B。建议任务 2 阶段按 2A→2→2B，主框架后按任务 4→5，细分排期尚非用户确定。原整体验收针对当时已有功能，后续增量另验收，不把尚未开发的权限作为任务 0 验收条件。
...
## 8. 可直接交给新 session 的说明（当前仅文档评审）

> 继续梳理 `/Users/gaohongchong/Documents/Codex/LJQCapp` 的需求，当前不要开始功能开发。按 AGENTS.md 阅读交接与任务书，从 docs/TASK_BOOKS.md 查看拆分范围和依赖，结合 docs/requirements_merged_2026_09_21.md 评审业务口径；用户另行要求开发时，仍从任务 0 开始。原 0—3 保留，新增 2A 基础资料、2B 临时质控、4 账号权限、5 数据维护与联动重算。保留工作区及已完成弹窗，核心算法不改，不操作真实用户数据库。实验室复用系统设置；账号关系中的建议不冒充已确认范围。更正/补录与对应重算必须完整交付，不把旧测试记录当作新增功能的验证。
