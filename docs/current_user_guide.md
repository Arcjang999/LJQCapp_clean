...
更新：2026-09-21。对应首页“使用指南”；适用于项目中心与弹窗交互版本。本文只说明当前已实现功能，后续标准扩展见 [任务书](task_00_standards_expansion.md)。
...
- 有适用标准时采用标准要求；标准候选不能直接跳过。不适用时逐条说明具体依据；没有适用条目时填写实验室自定要求、来源、版本和确认人。
- 当前内置 94 条要求来自 WS/T 403—2024 和 WS/T 406—2024。403 包含部分免疫球蛋白、激素和肿瘤标志物项目；406 包含血液和凝血项目。WS/T 属于卫生行业标准，来源按实际名称显示。
- 核对检验项目、方法学、单位、结果类型和浓度范围。Ct、log 或定性结果不能直接套用浓度尺度 CV 限值；当前目录未找到条目，不等于该项目没有国家或行业标准。
- 本版自动比较适用的实测 CV；SD、偏倚、总误差等要求供查阅。达到 CV 要求不等同于本次检测在控。
- 已确认批次保留原采用版本。复制或更换质控品后，需重新核对新批各水平的质量目标。

分子、定性免疫等标准的进一步收录和适用条件校验已列入后续开发，当前不表示这些标准已全部内置。
